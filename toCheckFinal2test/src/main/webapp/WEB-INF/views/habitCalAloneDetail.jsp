	      ...
	      <col width="188px">
	         <tr>
	            <td colspan="3" style="text-align: right;">
                	<input type="submit" class="btn btn-default btn-xs" value="Chcek Complete"/>
                    <input type="button" class="btn btn-default btn-xs" value="Delete" onclick="location.href='habitCalDeleteAlone.do?pKey=${dto.pKey}&id=${dto.id}&which=main'"/>
	         ...
	         <td>
		         <c:choose>
	                <c:when test="${(i==map.stYear&&j==map.stMonth&&n<map.stDate)||(i==map.edYear&&j==map.edMonth&&n>map.edDate)}">
		            	<p class="countview" style="color:${Util.fontColor(dayOfWeek.dayOfWeek(i,j,1), n)};">${n}</p>
	                </c:when>
					<c:otherwise>
		                ...
		                ${Util.checked(chkList, Util.substring(i),Util.isTwo(j+""),Util.isTwo(n+""))} 
